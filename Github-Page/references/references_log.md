...
> **Cập nhật chuẩn hóa lần cuối**: 2026-09-10 (Đã hoàn thành rà soát chéo 100% metadata, Crossref DOIs, arXiv IDs và tách bạch đóng góp gốc vs. mục tiêu thực nghiệm của đồ án).  
...
> 4. **ĐIỀU KIỆN TIẾP NHẬN TÀI LIỆU MỚI (New Reference Ingestion Criteria)**:
...
| :--- | :---: | :--- | :--- | :--- | :--- |
| **1. Tổng quan Kiến trúc LLM & Lỗ hổng Ranh giới Phẳng** | <a href="#ref1">`[1]`</a> | Zhao et al. (2023) | **`Zhao_2023_A_Survey_of_Large_Language_Models.pdf`** | Khảo sát toàn diện kiến trúc Transformer tự hồi quy và không gian token phẳng $X = S \mathbin{\Vert} U$. | Cơ sở lý luận: LLM không thể tự phân tách lệnh và dữ liệu. (Chương 1, 2) |
| **2. Instruction Tuning & Xử lý System Prompt** | <a href="#ref2">`[2]`</a> | Ouyang et al. (2022) | **`Ouyang_2022_InstructGPT_Training_Language_Models_Follow_Instructions.pdf`** | Nguyên lý RLHF; chứng minh thiên kiến tối ưu tuân thủ mệnh lệnh người dùng. | Giải thích cơ chế xung đột giữa System Prompt và User Prompt. (Chương 1, 2) |
| **3. Direct Prompt Injection (Tấn công Trực tiếp)** | <a href="#ref3">`[3]`</a> | Perez & Ribeiro (2022) | **`Perez_2022_Ignore_This_Title_Hack_This_Paper_Prompt_Injection.pdf`** | Định nghĩa và phân loại chính thức: Goal Hijacking và Prompt Leaking. | Phân loại tấn công trực tiếp; xây dựng kịch bản kiểm thử Demo 1. (Chương 1, 2, 3) |
| **4. Indirect Prompt Injection (Tấn công Gián tiếp)** | <a href="#ref4">`[4]`</a> | Greshake et al. (2023) | **`Greshake_2023_Indirect_Prompt_Injection.pdf`** | Độc hại nhúng trong dữ liệu bên ngoài (Web/RAG); mô hình hóa rủi ro ứng dụng. | Luận giải nhu cầu bắt buộc phải có lớp Input Guardrail độc lập ở Ingress. (Chương 1, 2, 3) |
| **5. Cơ chế Thất bại Căn chỉnh An toàn (Jailbreak Failures)** | <a href="#ref5">`[5]`</a> | Wei et al. (2024) | **`Wei_2024_Jailbroken_How_LLM_Safety_Training_Fails.pdf`** | Xác lập 2 chế độ lỗi căn chỉnh: Competing Objectives & Mismatched Generalization. | Cơ sở chứng minh an toàn nội tại là chưa đủ, cần bộ phân loại độc lập. (Chương 1, 4) |
...
| **15. Phòng Thủ Cơ Bản Bằng Thống Kê Chuỗi & Cú Pháp** | <a href="#ref15">`[15]`</a> | Jain et al. (2023) | **`Jain_2023_Baseline_Defenses_Adversarial_Attacks_LLMs.pdf`** | Chứng minh các bộ lọc đơn giản (Perplexity, n-gram) phát hiện hiệu quả chuỗi đối kháng. | Cơ sở lý luận cho Tầng 1 của PI-Guard: Sử dụng Classical ML TF-IDF Word/Char để lọc nhanh cú pháp. (Chương 3, 4) |
| **16. Lượng Hóa Động Sau Huấn Luyện (PTQ) Cho Transformer** | <a href="#ref16">`[16]`</a> | Yao et al. (2022) | **`Yao_2022_ZeroQuant_Efficient_Post_Training_Quantization_Transformers.pdf`** | Phương pháp lượng hóa ZeroQuant (weight INT8, token-wise activation INT8) suy hao thấp. | Cơ sở kỹ thuật để lượng hóa DeBERTa-v3 sang ONNX INT8 phục vụ suy luận CPU độ trễ thấp. (Chương 3, 5) |
| **17. Lẩn Tránh Bằng Mã Hóa Ký Tự (CipherChat & Encoding)** | <a href="#ref17">`[17]`</a> | Yuan et al. (2024) | **`Yuan_2024_GPT4_Too_Smart_To_Be_Safe_Cipher_Jailbreak.pdf`** | Khung CipherChat: Chứng minh năng lực giải mã của LLM có thể bị lợi dụng để vượt qua căn chỉnh an toàn. | Luận chứng cho việc tích hợp mô-đun tiền xử lý chuẩn hóa chuỗi và giải mã tiền trạm. (Chương 1, 3, 4) |
| **18. Nguyên Lý Thiết Kế Hệ Thống Bảo Vệ Kinh Điển** | <a href="#ref18">`[18]`</a> | Saltzer & Schroeder (1975) | **`Saltzer_1975_The_Protection_of_Information_in_Computer_Systems.pdf`** | 8 nguyên tắc bảo vệ kinh điển (Complete Mediation, Economy of Mechanism, Defense-in-Depth). | Nền tảng thiết kế kiến trúc: Kiểm soát toàn diện tại Ingress và cơ chế phân tầng nhẹ nhàng. (Chương 2, 3) |
...
- **Định vị kỹ thuật & Giả thuyết thực nghiệm của PI-Guard**: **Chương 1 (Giới thiệu vấn đề)** & **Chương 2 (Cơ sở lý thuyết)** — Nhóm PI-Guard sử dụng bài báo để luận giải nguyên nhân gốc rễ của tấn công tiêm nhiễm: downstream LLM vốn dĩ xử lý chuỗi $X = S \mathbin{\Vert} U$ mà không có cơ chế cách ly ranh giới cứng giữa System Prompt ($S$) và User Prompt ($U$), đòi hỏi phải có một lớp bảo vệ tiền trạm bên ngoài.
...
- **Đóng góp khoa học gốc của bài báo**: Giới thiệu phương pháp huấn luyện InstructGPT sử dụng học tăng cường từ phản hồi của con người (RLHF) trên các mô hình ngôn ngữ lớn để căn chỉnh mô hình tuân thủ mục đích và chỉ thị của người dùng (Helpful, Honest, Harmless).
- **Định vị kỹ thuật & Giả thuyết thực nghiệm của PI-Guard**: **Chương 1** & **Chương 2** — Nhóm viện dẫn bài báo để giải thích cơ chế xung đột lợi ích: Quá trình RLHF tối ưu hóa mạnh khả năng "tuân thủ chỉ thị người dùng" (instruction-following capability), vô tình khiến LLM ưu tiên thực hiện các lệnh tiêm nhiễm mới xuất hiện trong User Prompt hơn là duy trì ngữ cảnh kiểm soát tĩnh của nhà phát triển.
...
- **Tệp PDF Cục Bộ**: **`Greshake_2023_Indirect_Prompt_Injection.pdf`** (12 trang)
...
- **Tệp PDF Cục Bộ**: **`Tencent_2026_AI_Infra_Guard_MultiLayer_Agent_RedTeaming.pdf`** (42 trang)
...
- **Tệp PDF Cục Bộ**: **`Meta_2023_Llama_Guard_Input_Output_Safeguard.pdf`** (15 trang)
...
- **Tệp PDF Cục Bộ**: **`He_2023_DeBERTaV3_Disentangled_Attention_ICLR.pdf`** (18 trang)
...
- **Tệp PDF Cục Bộ**: **`Zhou_2024_EasyJailbreak_Unified_Framework.pdf`** (9 trang)
...
- **Tệp PDF Cục Bộ**: **`Jain_2023_Baseline_Defenses_Adversarial_Attacks_LLMs.pdf`** (20 trang)
...
- **Đóng góp khoa học gốc của bài báo**: Khảo sát và thực nghiệm các phương pháp phòng vệ đơn giản (bộ lọc Perplexity, diễn giải lại câu, tái mã hóa token) chống lại các tấn công đối kháng; chứng minh các phương pháp dựa trên phân tích đặc trưng cú pháp và độ hỗn loạn chuỗi ký tự có thể phát hiện hiệu quả nhiều dạng tấn công tối ưu hóa mà chi phí tính toán rất thấp.
- **Định vị kỹ thuật & Giả thuyết thực nghiệm của PI-Guard**: **Chương 3** & **Chương 4** — Cung cấp nền tảng lý luận cho việc xây dựng Tầng 1 (Classical ML TF-IDF Word + Character n-grams) trong kiến trúc phòng thủ phân tầng của PI-Guard, chứng minh các phương pháp phân tích cú pháp ký tự có giá trị phòng ngự thực tế cao đối với các biến dạng văn bản bất thường.
...
- **Tệp PDF Cục Bộ**: **`Yao_2022_ZeroQuant_Efficient_Post_Training_Quantization_Transformers.pdf`** (24 trang)
...
- **Tệp PDF Cục Bộ**: **`Saltzer_1975_The_Protection_of_Information_in_Computer_Systems.pdf`** (54 trang)
...
- **Đóng góp khoa học gốc của bài báo**: Tác phẩm kinh điển nền tảng của ngành An toàn Thông tin, định hình 8 nguyên tắc thiết kế hệ thống bảo vệ máy tính (Economy of Mechanism, Fail-Safe Defaults, Complete Mediation, Open Design, Separation of Privilege, Least Privilege, Least Common Mechanism, Psychological Acceptability).
