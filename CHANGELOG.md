...
- **Internal Guidelines Governance & Fall 2026 Milestone Independence**: Documented across all Agent Skills (`fpt-capstone-rubrics-and-process`, `capstone-thesis-and-defense`, `review1-threat-model-and-defense`), repository governance rules (`AGENTS.md`, `CONTRIBUTING.md`, `capstone-project-guidelines.md`), and thesis rubrics summary that `docs/fpt_capstone_guide/` is confidential university reference material (guarded locally with a dedicated `README.md` and gitignored). Formally codified that the 4-Milestone Timeline for Fall 2026 is an independent, customized schedule approved by Supervisor MSc. Trần Văn Ninh rather than inherited from legacy semester archives.
