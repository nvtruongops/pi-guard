...
> 2. **Trạng thái Review 1 (Tuần 1–4)**: Duy trì đầy đủ khung cấu trúc phân hệ (module scaffolding) với các thư mục chức năng và `.gitkeep`. Tuyệt đối không chứa code dang dở.
> 3. Thành viên **KHÔNG ĐƯỢC CODE TRỰC TIẾP HOẶC TEST NHÁP TRONG `Final-Report/src/`**.
> 4. Mọi tính năng mới, thử nghiệm thuật toán phải viết trong `workspaces/<tên_bạn>/` trước $\rightarrow$ Sau khi họp nhóm cuối tuần thống nhất và nghiệm thu mới được Leader merge vào `Final-Report/src/` tại cột mốc Review 2 và Review 3.
